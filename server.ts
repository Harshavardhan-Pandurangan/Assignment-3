...
restoreOverwrittenFilesWithOriginals()
    .then(() => {
        ...
        app.use((req: Request, res: Response, next: NextFunction) => {
            res.append(
                "X-Recruiting",
                config.get("application.securityTxt.hiring")
            );
            next();
        });
        ...
        securityTxtExpiration.setFullYear(
            securityTxtExpiration.getFullYear() + 1
        );
        app.get(
        ...
        app.use(
            ...
            securityTxt({
                ...
                "Preferred-Languages": [
                    ...new Set(
                        locales.map((locale: { key: string }) =>
                            locale.key.substr(0, 2)
                        )
                    ),
                ].join(", "),
            ...
            if (name === "Challenge") {
                    ...
                    (
                        ...
                        for (let i = 0; i < context.instance.length; i++) {
                            ...
                            if (context.instance[i].hint) {
                                context.instance[i].hint = req.__(
                                    context.instance[i].hint
                                );
                            }
                        }
                ...
                resource.read.send.before(
                    (
                        ...
                        context.instance.description = req.__(
                            context.instance.description
                        );
                        if (context.instance.hint) {
                            context.instance.hint = req.__(
                                context.instance.hint
                            );
                        }
                        return context.continue;
                    }
            ...
            if (name === "SecurityQuestion") {
                ...
                resource.read.send.before(
                    (
                        ...
                        context.instance.question = req.__(
            ...
            if (name === "Product") {
                resource.list.fetch.after(
                    (
                        ...
                        for (let i = 0; i < context.instance.length; i++) {
                            context.instance[i].name = req.__(
                                context.instance[i].name
                            );
...
while (
    ...
    logger.info(
        `Entity models ${colors.bold(
            Object.keys(sequelize.models).length.toString()
        )} of ${colors.bold(
            expectedModels.length.toString()
        )} are initialized (${colors.yellow("WAITING")})`
    );
}
logger.info(
    `Entity models ${colors.bold(
        ...
        expectedModels.length.toString()
    )} are initialized (${colors.green("OK")})`
);
...
errorhandler.title = `${config.get<string>(
    "application.name"
